...
try {
    ...
    if ($existingDoc) {
        ...
        // Get next number
        $stmt = $pdo->prepare("SELECT last_number FROM document_numbers WHERE document_type = ? AND year = ? AND month = ?");
        $stmt->execute([$documentType, $year, $month]);
        $row = $stmt->fetch();
        $nextNum = $row ? $row['last_number'] + 1 : 1;
        
        // Generate document number
        $generatedNumber = sprintf('%03d/%s/UPT Manado/%s/%d', $nextNum, $typeCode, $romanMonth, $year);
        
        // If a document number was provided, use it
        if (!empty($documentNumber)) {
            $generatedNumber = $documentNumber;
            // Extract number from provided document number
        ...
        // Update/insert document_numbers
        ...
        $stmt->execute([$documentType, $year, $month, $nextNum]);
